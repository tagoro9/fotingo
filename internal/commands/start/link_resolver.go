...
func ResolveIssueLink(client IssueLinkClient, opts ResolveIssueLinkOptions) (string, error) {
	...
	for {
		...
		if issueID, ok := extractIssueIDFromJiraBrowseURL(currentQuery, opts.JiraRoot); ok {
			currentQuery = issueID
			normalizedQuery = issueID
		}
...
func extractIssueIDFromJiraBrowseURL(raw string, jiraRoot string) (string, bool) {
	...
	parsedRoot, err := url.Parse(normalizedRoot)
	if err != nil {
		return "", false
	}

	parsedQuery, err := url.Parse(strings.TrimSpace(raw))
	if err != nil || parsedQuery.Scheme == "" || parsedQuery.Host == "" {
		return "", false
	}
	if parsedQuery.Scheme != "https" || !strings.EqualFold(parsedQuery.Host, parsedRoot.Host) {
		return "", false
	}

	pathParts := strings.Split(strings.Trim(parsedQuery.Path, "/"), "/")
	if len(pathParts) != 2 || pathParts[0] != "browse" || strings.TrimSpace(pathParts[1]) == "" {
		return "", false
	}

	return strings.ToUpper(strings.TrimSpace(pathParts[1])), true
}
