...
func (r WorkflowRunner) Run(statusCh *chan string, out WorkflowEmitter, allowEditor bool) WorkflowResult {
	...
	if len(r.Options.Labels) > 0 {
		resolvedLabels, missingLabels, resolveErr := r.Deps.ResolveLabels(ghClient, r.Options.Labels)
		if resolveErr != nil {
			result.Err = fterrors.WrapGitHubError(t(i18n.ReviewWrapResolveLabels), resolveErr)
			return result
		}
	...
	if len(r.Options.Reviewers) > 0 {
		resolvedReviewers, resolvedTeamReviewers, resolveWarnings, resolveErr := r.Deps.ResolveReviewers(ghClient, r.Options.Reviewers)
		if resolveErr != nil {
			result.Err = fterrors.WrapGitHubError(t(i18n.ReviewWrapResolveReviewers), resolveErr)
			return result
		}
	...
	if len(r.Options.Assignees) > 0 {
		resolvedAssignees, resolveWarnings, resolveErr := r.Deps.ResolveAssignees(ghClient, r.Options.Assignees)
		if resolveErr != nil {
			result.Err = fterrors.WrapGitHubError(t(i18n.ReviewWrapResolveAssignees), resolveErr)
			return result
		}
		for _, warning := range resolveWarnings {
			out.InfoRaw("warning", warning)
	...
	out.Info("success", i18n.ReviewStatusSuccess, pr.HTMLURL)
	return result
}
