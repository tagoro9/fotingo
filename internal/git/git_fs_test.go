...
func TestNew_UsesWorktreeRootForCredentials(t *testing.T) {
	tmpDir := t.TempDir()
	repoDir := filepath.Join(tmpDir, "repo")
	linkedDir := filepath.Join(tmpDir, "linked")
	helperPath := filepath.Join(tmpDir, "credential-helper.sh")
	...
	runGitCommand(t, repoDir, "branch", "feature")
	runGitCommand(t, repoDir, "worktree", "add", linkedDir, "feature")
	runGitCommand(t, repoDir, "config", "extensions.worktreeConfig", "true")
...
`), 0755))
	...
	runGitCommand(t, linkedDir, "config", "--worktree", "credential.helper", "!"+helperPath)
	runGitCommand(t, linkedDir, "remote", "add", "origin", "https://github.com/tagoro9/fotingo-playground.git")

	t.Chdir(linkedDir)

	cfg := config.NewDefaultConfig()
	messages := make(chan string, 1)
	client, err := New(cfg, &messages)
	require.NoError(t, err)

	internalGit, ok := client.(*git)
	require.True(t, ok)

	provider, ok := internalGit.credentialProvider.(*execCredentialProvider)
	require.True(t, ok)
	assert.Equal(t, linkedDir, provider.dir)

	remote, err := internalGit.repo.Remote("origin")
	require.NoError(t, err)
	require.Len(t, remote.Config().URLs, 1)
	assert.Equal(t, "https://github.com/tagoro9/fotingo-playground.git", remote.Config().URLs[0])

	credentials, err := provider.GetCredentials("https://github.com/tagoro9/fotingo-playground.git")
	require.NoError(t, err)
	assert.Equal(t, "test-user", credentials.Username)
