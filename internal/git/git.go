...
func (g *git) GetRemote() (giturl.IGitURL, error) {
	...
	gitUrl, err := giturl.NewGitURL(remote.Config().URLs[0])
	if err != nil {
		return nil, err
	}
	return gitUrl, nil
...
func (e *execCredentialProvider) GetCredentials(remoteURL string) (*http.BasicAuth, error) {
	...
	if username == "" || password == "" {
		return nil, fmt.Errorf("no credentials found for %s", remoteURL)
	}
...
func (g *git) fetch(remote string) error {
	remoteRef, err := g.repo.Remote(remote)
	if err != nil {
		return fmt.Errorf("failed to get remote %s: %w", remote, err)
	}

	credentials, err := g.credentialProvider.GetCredentials(remoteRef.Config().URLs[0])
	if err != nil {
...
func (g *git) getRemoteDefaultBranchFromGitCommand(remote string) (string, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		return "", fmt.Errorf("failed to inspect remote %s: %s", remote, stderrOutput)
	}

	branch, err := parseRemoteHeadBranch(stdout.String())
	if err != nil {
...
func (g *git) DoesBranchExistInRemote(branch string) (bool, error) {
	...
	remoteRef, err := g.repo.Remote(remote)
	if err != nil {
		return false, fmt.Errorf("failed to get remote %s: %w", remote, err)
	}
...
func New(cfg *viper.Viper, messages *chan string) (Git, error) {
	...
	return &git{
		...
		repo:                     repo,
		messages:                 messages,
		credentialProvider:       &execCredentialProvider{},
	}, nil
}
