  ...
  function fetchOptions() {
    ...
    return {
      callbacks: {
        ...
        credentials: _ramda2['default'].compose(_nodegit2['default'].Cred.sshKeyFromAgent, (0, _util2.debugCurried)('git', 'Getting authentication from SSH agent'), function (username) {
...
}), _ramda2['default'].nthArg(0)), _ramda2['default'].compose(_ramda2['default'].flatten, _ramda2['default'].map(_ramda2['default'].compose(_ramda2['default'].map(function (ref) {
  return { raw: '' + ref.prefix + ref.issue, issue: ref.issue };
}), _ramda2['default'].prop('references'))), _ramda2['default'].nthArg(1))]);
...
exports['default'] = {
  ...
  createIssueBranch: _ramda2['default'].curryN(2, function (config, name) {
    ...
    return repository.fetch(remote, fetchOptions())['catch']((0, _error.throwControlledError)(_error.errors.git.noSshKey)).then((0, _util2.debugCurriedP)('git', 'Getting local repository status')).then(function () {
    ...
    }).then((0, _util2.debugCurriedP)('git', 'Creating new branch')).then(_ramda2['default'].compose((0, _error.catchPromiseAndThrow)('git', function (e) {
      switch (e.errno) {
        case _nodegit2['default'].Error.CODE.EEXISTS:
          return _error.errors.git.branchAlreadyExists;
        default:
          throw e;
      }
    }), function (commit) {
  ...
  pushBranchToGithub: _ramda2['default'].converge(_ramda2['default'].composeP(function (_ref2) {
    ...
    return remote.push([ref], fetchOptions())['catch']((0, _error.throwControlledError)(_error.errors.git.noSshKey)).then(function () {
  ...
  extractIssueFromCurrentBranch: function () {
    function extractIssueFromCurrentBranch() {
      return _ramda2['default'].composeP((0, _util2.debugCurriedP)('git', 'Extracting issue from current branch'), _ramda2['default'].compose(_util2.wrapInPromise, _ramda2['default'].when(_ramda2['default'].isNil, (0, _error.throwControlledError)(_error.errors.git.noIssueInBranchName)), _util.getIssueIdFromBranch), getCurrentBranchName)();
    }

    return extractIssueFromCurrentBranch;
  }(),
  getBranchInfo: function () {
    function getBranchInfo(config, issue) {
      (0, _util2.debug)('git', 'Getting branch commit history');

      var _config$get2 = config.get(['git']),
          remote = _config$get2.remote,
          branch = _config$get2.branch;

      return Promise.all([repository.getHeadCommit(), repository.getBranchCommit(remote + '/' + branch)]).then(_ramda2['default'].when(_ramda2['default'].compose(_ramda2['default'].not, _ramda2['default'].allUniq, _ramda2['default'].map(_ramda2['default'].compose(_ramda2['default'].toString, _ramda2['default'].invoker(0, 'id')))), (0, _error.throwControlledError)(_error.errors.git.noChanges))).then(function (_ref4) {
