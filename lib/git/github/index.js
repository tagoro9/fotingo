...
var buildPullRequestTitle = _ramda2['default'].ifElse(_ramda2['default'].isNil, _ramda2['default'].compose(_ramda2['default'].concat(_ramda2['default'].__, '\n'), _ramda2['default'].nthArg(1)), function (issue) {
  return _util.ISSUE_TYPES[issue.fields.issuetype.name] + '/' + issue.key + ' ' + _ramda2['default'].take(60, issue.fields.summary) + '\n';
});

// Array -> String
var buildPullRequestBody = _ramda2['default'].compose(_ramda2['default'].join('\n'), _ramda2['default'].map(_ramda2['default'].compose(_ramda2['default'].join('\n'), _ramda2['default'].converge(_ramda2['default'].prepend, [_ramda2['default'].compose(_ramda2['default'].concat('* '), _ramda2['default'].head), _ramda2['default'].compose(_ramda2['default'].map(_ramda2['default'].concat('  ')), _ramda2['default'].tail)]), _ramda2['default'].split('\n'), _ramda2['default'].prop('message'))));
// Object -> Array -> String
var buildPullRequestFooter = function () {
  function buildPullRequestFooter(issueRoot) {
    return _ramda2['default'].compose(_ramda2['default'].concat('\nFixes '), _ramda2['default'].join(', '), _ramda2['default'].map(function (issue) {
      return '[' + issue + '](' + issueRoot + _ramda2['default'].tail(issue) + ')';
    }), _ramda2['default'].uniq, _ramda2['default'].flatten, _ramda2['default'].map(_ramda2['default'].ifElse(_ramda2['default'].has('key'), _ramda2['default'].compose(_ramda2['default'].concat('#'), _ramda2['default'].prop('key')), _ramda2['default'].prop('issues'))), _ramda2['default'].reject(_ramda2['default'].either(_ramda2['default'].isEmpty, _ramda2['default'].isNil)), _ramda2['default'].prepend);
  }
...
var buildPullRequestDescription = function () {
  function buildPullRequestDescription(issueRoot) {
    return _ramda2['default'].converge(_ramda2['default'].compose(_util2.wrapInPromise, _ramda2['default'].unapply(_ramda2['default'].join('\n'))), [buildPullRequestTitle, _ramda2['default'].compose(buildPullRequestBody, _ramda2['default'].nthArg(2)), _ramda2['default'].compose(_ramda2['default'].apply(buildPullRequestFooter(issueRoot)), _ramda2['default'].unapply(_ramda2['default'].remove(1, 1)))]);
  }
...
exports['default'] = {
  ...
  createPullRequest: _ramda2['default'].curryN(5, function (config, project, issue, issueRoot, branchInfo) {
    ...
    _ramda2['default'].composeP(_ramda2['default'].converge(function (issues, pullRequest) {
      return { issues: issues, pullRequest: pullRequest };
    }, [_ramda2['default'].compose(_ramda2['default'].ifElse(_ramda2['default'].partial(_ramda2['default'].isNil, [issue]), _ramda2['default'].identity, _ramda2['default'].append(issue)), _ramda2['default'].map(_ramda2['default'].compose(_ramda2['default'].set(_ramda2['default'].lensProp('key'), _ramda2['default'].__, {}), _ramda2['default'].tail)), _ramda2['default'].flatten, _ramda2['default'].map(_ramda2['default'].prop('issues')), _ramda2['default'].view(_ramda2['default'].lensPath(['branchInfo', 'commits']))), _ramda2['default'].prop('pullRequest')]), _ramda2['default'].compose(_util2.wrapInPromise, _ramda2['default'].set(_ramda2['default'].lensProp('pullRequest'), _ramda2['default'].__, { branchInfo: branchInfo })), submitPullRequest(config, project, branchInfo))), (0, _util2.debugCurriedP)('github', 'Submitting pull request to github'), allowUserToEditPullRequest, (0, _util2.debugCurriedP)('github', 'Editing pull request description'), buildPullRequestDescription(issueRoot))(issue, branchInfo.name, branchInfo.commits);
  })
};