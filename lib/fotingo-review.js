...
try {
  (function () {
    _commander2['default'].option('-n, --no-branch-issue', 'Do not pick issue from branch name').option('-s, --simple', 'Do not use any issue tracker').option('-l, --label [labels]', 'Labels to add to the PR', _ramda2['default'].append, []).parse(process.argv);
    ...
    (0, _init2['default'])(_config2['default'], _commander2['default']).then(function (_ref) {
      ...
      return git.getCurrentBranchName().then(_ramda2['default'].compose(_util2.wrapInPromise, stepCurried(2, function (name) {
        ...
        return git.getBranchInfo(_config2['default'], issue).then(step(6 - stepOffset, 'Creating pull request', 'speaker')).then(github.checkAndGetLabels(_config2['default'], project, _commander2['default'].label)).then(github.createPullRequest(_config2['default'], project, issue, issueTracker.issueRoot, {
          addLinksToIssues: !_commander2['default'].simple
        }));
