...
try {
  (function () {
    ...
    (0, _init2['default'])(_config2['default'], _commander2['default']).then(function (_ref) {
      ...
      return (0, _util2.wrapInPromise)(step(2, 'Pushing current branch to Github', 'arrow_up')).then(git.pushBranchToGithub(_config2['default'])).then(_ramda2['default'].ifElse(shouldGetIssue, _ramda2['default'].compose(issueTracker.getIssue, stepCurried(4, 'Getting issue from ' + issueTracker.name, 'bug'), git.extractIssueFromCurrentBranch, stepCurried(3, 'Extracting issue from current branch', 'mag_right')), _ramda2['default'].always(undefined))).then(function (issue) {
