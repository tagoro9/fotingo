...
const buildPullRequestBody = R.compose(
  ...
  R.map(
    R.converge(R.concat, [
      ...
      R.compose(R.ifElse(R.isNil, R.always(''), R.replace(/\n|^/g, '\n  ')), R.prop('body')),
    ]),
  ),
);
...
const buildPullRequestDescription = (issueRoot, addLinksToIssues) =>
  R.converge(R.compose(wrapInPromise, R.unapply(R.join('\n'))), [
...
export default {
  ...
  checkAndGetLabels: R.curryN(4, (config, project, labels, branchInfo) =>
    R.composeP(
      ...
      R.unless(
        R.compose(names => R.all(R.contains(R.__, names), labels), R.map(R.prop('name'))),
