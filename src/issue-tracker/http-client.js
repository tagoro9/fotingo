...
const handleServerResponse = (resolve, reject) => R.ifElse(
  ...
  R.ifElse(
    ...
    R.compose(reject, R.unapply((args) => {
...
export default function (rootUrl) {
  ...
  const serverCall = R.curry((method, url, { form, body }) =>
    new Promise((resolve, reject) => {
      debug('http', `Performing ${method} ${makeUrl(url)} ${body ? `with body ${JSON.stringify(body, null, 2)}` : ''}`);
      return request(
  ...
  return {
    ...
    setAuth: ({ login, password }) => { auth = { ...auth, user: login, pass: password }; },
