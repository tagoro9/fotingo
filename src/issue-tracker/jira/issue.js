...
export default status => {
  ...
  // issue -> login -> Boolean
  const isInUserGroup = R.converge((groups, labels) => R.any(R.contains(R.__, groups), labels), [
  ...
  const hasValidStatus = R.compose(
    R.contains(R.__, [status.SELECTED_FOR_DEVELOPMENT, status.BACKLOG]),
    Number,
    R.path(['fields', 'status', 'id']),
  );
